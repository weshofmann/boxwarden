...
// AddressResolver performs one fresh DHCP-backed Tart lookup without caching
// an address as backend identity.
...
func NewAddressResolver(runner execx.Runner, tartPath, tartHome string) AddressResolver {
	return AddressResolver{runner: runner, tartPath: tartPath, tartHome: tartHome}
}
...
func (r AddressResolver) Resolve(ctx context.Context, objectID string) (string, error) {
	...
	if r.runner == nil || !canonicalAbsolutePath(r.tartPath) || !canonicalAbsolutePath(r.tartHome) {
		return "", fmt.Errorf("resolve Tart address: qualified runner, Tart path, and Tart home are required")
	}
	commandContext, cancel := context.WithTimeout(ctx, addressCommandLimit)
	defer cancel()
	result, err := r.runner.Run(commandContext, execx.Command{
		Path: r.tartPath,
		Args: []string{"ip", "--resolver=dhcp", "--wait=60", objectID},
		Env:  tartCommandEnvironment(r.tartHome),
	})
	if err != nil {
		return "", fmt.Errorf("resolve Tart address: %w", err)
	}
