...
// Runner is argv-only. The helper never provides an ambient shell surface.
...
type Bootstrapper struct {
	...
	HostKeyPath string
	ZonePath    string
	Failpoint   func(string) error // deterministic test interruption only
}
...
func (b *Bootstrapper) guestPath(path string) (string, error) {
	return b.path(strings.TrimPrefix(path, "/"))
}
func (b *Bootstrapper) readGuestFile(path string, mode fs.FileMode) ([]byte, error) {
	full, err := b.guestPath(path)
	if err != nil {
		return nil, err
	}
...
func (b *Bootstrapper) validateBootstrapAncestors() error {
	...
	for _, path := range []string{"etc", "etc/ssh", boxwardenRelative} {
		...
		if !info.IsDir() || info.Mode()&os.ModeSymlink != 0 || info.Mode().Perm() != 0o755 || info.Mode().Perm()&0o022 != 0 || !ownedByCurrentUser(info) {
...
func (b *Bootstrapper) publishActive(parent, active string, request SerialRequest) error {
	...
	manifest := bindingManifest{Version: Version, Association: request.Association, CAFingerprint: request.CAFingerprint, Principal: request.Principal}
	...
	if err := writeSynced(filepath.Join(stage, "management-binding.json"), append(encoded, '\n'), 0o600); err != nil {
		return err
	}
	...
	// Linux production uses renameat2(RENAME_NOREPLACE); it is the final
	// no-replace guard after all state has been flushed and verified.
...
func verifyActive(active string, request SerialRequest) error {
	...
	if manifest.Version != Version || manifest.Association != request.Association || manifest.CAFingerprint != request.CAFingerprint || manifest.Principal != request.Principal || string(ca) != strings.TrimSpace(request.CAPublicKey)+"\n" || string(principal) != request.Principal+"\n" {
...
func readManifest(path string) (bindingManifest, error) {
	...
	if decodeFields(fields, &m) != nil {
		return bindingManifest{}, fmt.Errorf("invalid binding manifest")
	}
	if !m.Association.valid() || !validFingerprint(m.CAFingerprint) || m.Principal != derivedPrincipal(m.SessionID) {
		return bindingManifest{}, fmt.Errorf("invalid binding manifest")
	}
	return m, nil
}
...
func safeTree(active string) error {
	...
	if err := safeDirectory(filepath.Join(active, "authorized_principals"), 0o755); err != nil {
		return err
	}
	return nil
}
func rejectStaleStaging(parent string) error {
	...
	for _, e := range entries {
		if strings.HasPrefix(e.Name(), ".boxwarden-staging-") {
...
func safeDirectory(path string, mode fs.FileMode) error {
	...
	if !info.IsDir() || info.Mode()&os.ModeSymlink != 0 || info.Mode().Perm() != mode || info.Mode().Perm()&0o022 != 0 {
		return fmt.Errorf("unsafe directory")
	}
	if !ownedByCurrentUser(info) {
		return fmt.Errorf("directory is not owned by the helper user")
	}
	return nil
}
func readSafeFile(path string, mode fs.FileMode) ([]byte, error) {
	...
	if !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 || info.Mode().Perm() != mode || info.Mode().Perm()&0o022 != 0 {
		return nil, fmt.Errorf("unsafe file")
	}
	if !ownedByCurrentUser(info) {
		return nil, fmt.Errorf("file is not owned by the helper user")
	}
