...
// protocol. It is separate from the host protocol so the guest never gains a
// handle to host domain state or private CA material.
package guestproto
...
func EncodeSerialFrame(request SerialRequest, result SerialResult) (string, string, error) {
	...
	begin := fmt.Sprintf("BOXWARDEN-BEGIN %s %s", request.Nonce, request.SessionID)
	end := fmt.Sprintf("BOXWARDEN-END %s %s %s", request.Nonce, request.SessionID, base64.StdEncoding.EncodeToString(encoded))
