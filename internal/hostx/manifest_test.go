...
func TestParseManifestRejectsUnknownFieldsAndWrongQualifiedIdentity(t *testing.T) {
	...
	legacy := strings.Replace(valid, `"version":2,"platform"`, `"version":1,"platform"`, 1)
	legacy = strings.Replace(legacy, `"macos_build":"25G83",`, ``, 1)
	if _, err := ParseManifest([]byte(legacy)); !errors.Is(err, ErrUnsupportedManifestVersion) {
		t.Fatalf("ParseManifest(v1) error = %v, want unsupported manifest version", err)
	}
