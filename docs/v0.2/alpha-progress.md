...
Updated: 2026-09-23 21:46 UTC. Launch-relative target: complete tested alpha
...
| Source verification | verified with limits | Full local Go suite, targeted basebuild/alphaprep race tests, and vet passed at the preceding tool-capability checkpoint. Targeted Go tests, all guest shell fixtures, syntax, and diff checks passed for the current finalizer correction. Hosted CI is unavailable |
| VM inventory | verified | Admitted `TART_HOME` has nine stopped alpha-owned objects plus one protected stopped historical object; exact names and states are in the private ownership manifest |
...
its journal is failed, its artifacts remain private evidence, and no cache
receipt was issued. A fixture reproduced the mismatch before the one-line
finalizer correction and passed afterward. A new candidate is required for
qualification. Disk headroom must be re-established before another large
installation; no failed evidence has been discarded.
...
- Alpha VMs: three owned stopped generic installer candidates and six owned
...
Publish the finalizer run-ID correction, then establish enough free-space
headroom for a fresh complete preparation and qualification run. Record actual
