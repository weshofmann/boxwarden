...
Updated: 2026-09-23 21:20 UTC. Launch-relative target: complete tested alpha
...
| Source verification | verified with limits | Full local Go suite, targeted basebuild/alphaprep race tests, and vet passed for the current tool-capability preflight. Hosted CI is unavailable |
...
| Automatic base preparation | first real attempt failed before VM creation | The public command invokes admitted host preflight, exact source checks, private builder, and fresh-clone qualifier. Attempt `alpha-attempt-12392c5e3b77d1850a06a57087507eea` failed while generating its builder verifier: the selected macOS LibreSSL lacks `passwd -6`. Its private failed journal and staged input remain; no VM was created or cache admitted. Current source adds bounded functional OpenSSL/xorriso probes before attempt reservation. A fresh attempt with the verified OpenSSL 3 executable remains |
...
The first public preparation invocation used the verified Ubuntu input and
tracked recipe after healthy host, stopped-VM, RAM, and disk checks. It stopped
at verifier generation before VM creation because macOS LibreSSL does not
support the required SHA-512 crypt mode. The ownership manifest records this
attempt as failed before VM creation; its journal and staged input are private
immutable failure evidence. The source correction now probes the exact
OpenSSL `passwd -6 -stdin` behavior and xorriso `-version` with closed,
bounded commands before reserving an attempt. A separately verified regular
OpenSSL 3 executable supports the mode and will be used for a fresh attempt.
...
Publish the pre-attempt tool-capability correction, then run the public
preparation command with the verified OpenSSL 3 executable on a fresh candidate
and record actual
