...
Updated: 2026-09-23 22:49 UTC. Launch-relative target: complete tested alpha
...
| Source verification | verified with limits | Full local Go suite passed after formatter promotion. Targeted workspacex/workspaceformat race tests, focused vet, and diff checks passed. Independent review confirmed the ambiguous post-rename retry and architecture-guard corrections. These are source and synthetic checks; hosted CI and real workspace VM qualification remain unavailable |
...
| Workspace lifecycle | source registry and handoff, real path missing | Volume-owned record, attachment/use transitions, private format journal, Linux ARM64 ext4 helper, and retained exact Tart disk/lock leases have targeted tests. Public start/stop serialize through an outer transition lock and release the session lock during supervisor waits, with exact-generation rechecks and separate review. The bounded registry reader resolves exact session attachments; Attach refuses a fifth volume or duplicate filesystem UUID. Creating records can now promote only through a freshly admitted verified formatter journal and exact raw file; post-rename retries recheck the same evidence. Guest formatter VM adapter, use-reservation/child lease wiring, public attach/rebuild/reattach, and fresh synthetic qualification remain |
...
registry before public attachment. Use
the Starting session record as the batch commit marker after all Uses are
durable; leave Stopping in place until exact reap and Use release. Recheck exact
durable bindings in the child before Tart opens a disk.
