...
Updated: 2026-09-23 19:54 UTC. Launch-relative target: complete tested alpha
...
| Automatic base preparation | source verified, public path missing | Installer launcher, seed/remaster, journal, and admission source pass tests. Cache reuse binds a stopped Tart bundle identity; build stages exact verified inputs; serial admission checks private ACLs. Source tests prove prepare precedes finalization and failure leaves no admitted cache. A typed package query now passes through the live exact-generation supervisor with READY checks around its pinned SSH call. Qualifier guest inspection, lifecycle composition, and public CLI integration remain |
...
Implement the trusted qualifier guest-inspection adapter over the typed
supervisor query and wire public automatic
