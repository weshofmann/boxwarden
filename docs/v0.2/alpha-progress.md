...
| Controlled export | Bounded host stream receiver and zero-NIC synthetic inspector boot/transport have tests. A source-only extension can mount a private 64 MiB ext4 copy read-only with journal replay disabled and report the digest of one fixed bounded file. Review found and the follow-up closed a standalone hardlink admission gap; its negative test passed. Focused Go/Python tests, Swift typecheck, and shell syntax checks passed; a live copy boot, real stopped-volume export, and public export remain pending. |
