...
| Reusable preparation | Strict versioned recipe and ISO checks, candidate build, guest preparation, fresh-clone qualifier, private evidence, and cache admission are implemented. The latest real recipe-bound build passed installation and guest preparation, then timed out waiting for the clone-ready marker during finalization. Its candidate stopped and its journal records failure. No prepared base has passed real qualification or entered the cache. |
...
checkpoint or resumable build. The finalizer failure is under diagnosis before
another build.
...
1. Diagnose the finalizer timeout from preserved failed-run evidence. Resolve
   the cause, verify the correction, and restore disk headroom before a fresh
