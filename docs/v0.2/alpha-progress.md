...
Updated: 2026-09-23 19:46 UTC. Launch-relative target: complete tested alpha
...
| Source verification | verified with limits | Full local Go tests and vet, targeted race tests for serial/basebuild, and four guest Python preparation tests passed at the 19:38 UTC snapshot. Package inspection guestproto/sshx targeted race tests and vet passed at 19:46 UTC. Guest shell fixtures and inspector source tests passed at earlier checkpoints. Hosted CI is unavailable |
...
| Automatic base preparation | source verified, public path missing | Installer launcher, seed/remaster, journal, and admission source pass tests. Cache reuse binds a stopped Tart bundle identity; build stages exact verified inputs; serial admission checks private ACLs. Source tests prove prepare precedes finalization and failure leaves no admitted cache. A typed exact-generation SSH package query now has source tests, but is not yet exposed through the supervisor or qualifier. Production guest inspector/lifecycle composition and public CLI integration remain |
...
Expose the typed package query through the live supervisor and implement the
trusted qualifier guest-inspection adapter; then wire public automatic
