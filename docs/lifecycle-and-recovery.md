...
The current Ubuntu source path is `guest/ubuntu-24.04-arm64/autoinstall/user-data`, `guest/ubuntu-24.04-arm64/render-golden-seed.sh`, and `guest/ubuntu-24.04-arm64/remaster-golden-iso.sh`. Given `RUN_ID` (`run-1` or `run-2`) and an operator-owned mode-`0600`/`0400` SHA-512-crypt hash file, run `render-golden-seed.sh RUN_ID PASSWORD_HASH_FILE NEW_PRIVATE_SEED_DIR`; it detects and validates the trusted host's current IANA zone without fallback. Then run `remaster-golden-iso.sh SOURCE.iso NEW_PRIVATE_SEED_DIR/user-data OUTPUT.iso`. The latter maps the locked bootstrap helper and `guest/ubuntu-24.04-arm64/finalize-golden.sh` into a newly assembled installer ISO; autoinstall verifies their exact digests and installs them at fixed guest paths. Rendered user-data and the installer ISO contain a temporary build-password verifier and remain sensitive build inputs until deliberately destroyed. The Task 0 `scripts/spike/` harness and its finalizer remain historical evidence, not the current golden-finalization procedure.
