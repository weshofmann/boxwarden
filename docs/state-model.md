...
Durable lifecycle identity is the tuple `(domain, session UUID, backend kind/object, intended state)`. A persisted start generation binds one transition to one supervisor instance. A minimal request records that binding plus configuration/session-record locators; it is an expected record, not authority. An ordinary generation lock arbitrates cooperating host owners, which retain live process and serial handles in memory. Nonce, serial state, address, generation key/certificate, and health snapshots are runtime state, not alternate guest identity. Host-key pins are durable trusted state bound to the exact domain/session/backend tuple and contain the public host key/fingerprint, never an IP. Eventual READY requires fresh exact-generation retained-child and serial health, a structurally valid exact backend listing, pin, current certificate, strict probe, and time-zone agreement; a Tart `stopped` listing may contradict a live retained generation and must remain visible. Backend listing alone or a persisted success bit is insufficient. An absent or unprovable owner is never adopted as READY, and a `stopped` listing alone does not authorize release of a launched generation. Slice A implements the foundation only; the A–H lifecycle plan tracks pending composition.
