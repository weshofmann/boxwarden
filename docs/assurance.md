...
| L13 | Current generic-golden source removes the temporary build verifier and clone/build identity, refuses active trust, and installs a fail-closed first-boot identity dependency | D, T | `guest/ubuntu-24.04-arm64/render-golden-seed.sh`; `guest/ubuntu-24.04-arm64/finalize-golden.sh`; `guest/ubuntu-24.04-arm64/tests/render-golden-seed.sh`; `guest/ubuntu-24.04-arm64/tests/finalize-golden.sh`; `guest/ubuntu-24.04-arm64/tests/remaster-golden-iso.sh`; `internal/guestproto/golden_definition_test.go` | Source/fixture evidence only. Fresh Phase 3 real-guest password/PAM, GUI/serial, systemd/shutdown, DHCP/DUID, and two-clone qualification remains pending |
