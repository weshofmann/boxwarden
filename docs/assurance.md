...
| L6 | A `starting + G` retry never allocates another generation; it reconnects to the matching live owner, waits for exact cleanup ownership, or finishes the exact validated crash residue before relaunching G | D, T | ADR 009; `docs/lifecycle-and-recovery.md`; `internal/session/start_test.go`; `internal/supervisor/generation_cleanup_test.go`; `internal/supervisor/minimal_test.go` | Deterministic interruption injection only; no real-host retry/power-loss exercise |
