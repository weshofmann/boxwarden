...
| **Softnet runtime behavior** (privilege transition/drop, closed env, signal handling) | **Pending** | Gate procedure approved; execution in progress on a separate machine |
...
- Softnet runtime behavior (privilege transition/drop, closed-environment
  execution, signal handling) — procedure approved, execution in progress
...
- `boxwarden init` installs the exact qualified Softnet 0.19.0 executable in a
  ...
  transition/drop and closed-environment execution) is in progress and is not
  yet complete. Until it is, the project is not production-ready.
